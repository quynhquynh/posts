...
class UserAction extends React.Component {
  ...
  handleUser = action => {
    this.setState(prevState => ({
      isSignedup:
        action === "signup" ? !prevState.isSignedup : prevState.isSignedup,
      isLoggedin:
        action === "login" ? !prevState.isLoggedin : prevState.isLoggedin
    }));
  };
  ...
  validate = () => {
    ...
    const errors = {
      username:
        up.findIndex(u => u.username === username) !== -1
          ? ""
          : "username not matched",
      password:
        up.findIndex(u => u.password === password) !== -1
          ? ""
